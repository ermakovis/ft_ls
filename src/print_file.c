...
/*   Updated: 2019/06/15 19:10:15 by tcase            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		print_file(t_ls **ls, int flags)
{
	...
	while (tmp->next)
	{
		if (tmp->next && (S_ISREG(tmp->next->mode) || S_ISLNK(tmp->next->mode)))
		{
			cur = tmp->next->next;
			tmp->next->next = NULL;
			flags ^= FL_REGFL;
			flags & FL_LNG ? print_detail(tmp->next, flags)\
				: print_brief(tmp->next, flags);
			flags ^= FL_REGFL;
