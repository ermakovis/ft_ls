...
/*   Updated: 2019/06/24 12:27:02 by tcase            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_ls.h"

static void	print_brief_calc_rows(t_ls *ls, int *rows, int *width, int flags)
{
	t_wsize		ws;
	int			size;
	int			col;
...
void		print_brief(t_ls *ls, int flags)
{
	...
	if (!(flags & FL_ONE))
	{
		print_brief_calc_rows(ls, &rows, &width, flags);
